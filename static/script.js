...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const table = document.getElementById('statusTable');
    const rows = Array.from(table.querySelectorAll('tbody tr'));
    const pagination = document.getElementById('pagination'); let currentPage = 1;
...
if (form) {
    ...
    newForm.addEventListener("submit", function(e) {
        ...
        fetch('/add_task', {
            ...
            body: JSON.stringify({name: name, time: time, path: path}),
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function addTaskToTable(task) {
    ...
    newRow.innerHTML = `
        ...
        <td class="cellbutton">
            <button title="Iníciar automação" class="iniciar" onclick="startTask('${task.name}')">INÍCIAR</button>
        </td>
    `;
